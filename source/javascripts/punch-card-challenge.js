function punch_card_challenge_class(options) {
  ...
  this.do_slide = function () {
    ...
    if (self.options.easy) {
      // Easy mode just uses powers of 2
      answer = (1 << self.question_id) % 16
      // var new_answer = 1 << Math.floor(Math.random() * 4)
      // answer = (new_answer == answer)? 0 : new_answer
    } else {
      //answer = Math.floor(Math.random() * 16)
      // Avoid repeats
      answer = (answer + Math.floor(Math.random()*16)) % 16
    }
