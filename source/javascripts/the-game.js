...
function GameBridgeClass(options) {
  ...
  this.Resize = function () {
    //find the top left position of the slide show
    //use left as margin size for the bottom left and right
    var offset = $(this.parent).offset()
    if (this.margin == null) {
      this.margin = offset.left
    }
    var topMargin = offset.top
    ...
    //resize game if it's been loaded
    if (this.canvas != null) {
  ...
  this.GameText = function (gameText) {
    ...
    const singleCategoryMode = gameText.categories.length === 1
    var menuData = []
    ...
    // REB - this is a hack to bypass the initial category select menu
    // when there is only a single category of game.
    if (singleCategoryMode) {
      var firstMenu = this.categoryMenus[_.keys(this.categoryMenus)[0]]
      // slide_show.set_caption("OK?")
      this.SetMenu(firstMenu)
    }
  }
